...
func init() {
	// 注册任务处理器
	worker.RegisterWorkerFunc(consts.TEST_TASK_TASK, executeTestTask)
}

// executeTestTask 执行测试任务任务
func executeTestTask(ctx context.Context, t *asynq.Task) error {
	// 获取任务参数
	data, err := glob2.GetParamters[TestTaskData](ctx, t)
	if err != nil {
		glob2.WithWorkLog().Errorf(ctx, "[%s] 获取参数失败: %v", consts.TEST_TASK_TASK, err)
		return err
	}
	
	// 可以在这里添加参数验证
	// if err := g.Validator().Data(data).Run(ctx); err != nil {
	//     glob2.WithWorkLog().Errorf(ctx, "[%s] 参数验证失败: %v", consts.TEST_TASK_TASK, err)
	//     return err
	// }
	
	glob2.WithWorkLog().Infof(ctx, "[%s] 开始执行任务, 参数: %+v", consts.TEST_TASK_TASK, data)
	...
	glob2.WithWorkLog().Infof(ctx, "[%s] 任务执行完成", consts.TEST_TASK_TASK)
	return nil
}
